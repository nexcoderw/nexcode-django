...
- Keep `.env.example.local` and `.env.example.production` synchronized with the variables each environment uses. Group related variables under a comment explaining their purpose, required conditions, and safe configuration guidance. Leave every assignment empty; never copy actual values.
- Add or rename an environment variable in its consumer and the relevant example file together. Preserve existing private values; do not print private files or interpolate secrets into shell commands or tool output.
...
- Ignore `.env` and all `.env.*` files in Git and Docker contexts, except the value-free `.env.example.local` and `.env.example.production`. Never stage a private environment file. Restrict private file permissions to their owner and grant deployment access only where required.
